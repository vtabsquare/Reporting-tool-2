...
def _date_expr(field, fmt):
    ...
    if f in mapping:return f"TRY_STRPTIME({q}, '{mapping[f]}')::DATE"
    return f'TRY_CAST({q} AS DATE)'

def _cast_expr(field, target, date_format='auto'):
    q=quote(field);t=str(target or 'text').lower()
    if t in ('text','string'):return f'CAST({q} AS TEXT)'
    if t in ('integer','whole','whole number','int'):return f'CAST({q} AS INTEGER)'
    if t in ('decimal','number','real','currency'):return f'CAST({q} AS DOUBLE)'
...
def suggest_calculated_column(prompt:str, columns:list[str]):
    ...
    # Date difference.
    if any(x in low for x in ('days between','difference between','date difference','datediff')):
    ...
    mentioned=[c for c in columns if norm(c) in n]
    op=None
    if any(x in low for x in ('multiply','product','times',' * ')):op='*'
    elif any(x in low for x in ('divide','ratio',' / ')):op='/'
    elif any(x in low for x in ('subtract','difference','minus',' - ')):op='-'
    elif any(x in low for x in ('add ','sum of',' plus ',' + ')):op='+'
    ...
    # Never silently generate unrelated logic. If intent cannot be resolved with high
    # confidence, fail safely and ask the author to clarify the business rule.
    c=mentioned[0] if mentioned else best(fallback=False)
    hint=f' The most likely referenced column is {c}.' if c else ''
    raise ValueError('I could not determine the requested row-level operation with enough confidence.'+hint+' Try a precise instruction such as: CustomerID first 3 characters from left; extract last 4 characters from OrderID; if Status is Done then Closed else Open; or days between OrderDate and ShipDate.')
    ...
    for pos,st in enumerate(steps):
        ...
        if typ=='select_columns':
            ...
            mapping={
              ...
              'clean':f'REPLACE(REPLACE(REPLACE(CAST({q} AS VARCHAR), chr(13), ''), chr(10), ' '), chr(9), ' ')',
            ...
            allcols=[set(_columns(t)) for t in tables];common=[c for c in cols if all(c in x for x in allcols)]
            if not common:raise ValueError('Append requires common column names.')
            sel=', '.join(quote(c) for c in common);parts=[f'SELECT {sel} FROM {current}']+[f'SELECT {sel} FROM {quote(t)}' for t in tables]
            sql=' UNION ALL '.join(parts);cols=common
        elif typ=='group_by':
...
def ai_suggestions(source,steps):
    ...
    for col in cols:
        ...
        name=col.lower()
        if ('date' in name or name.endswith('dt')) and strings:
