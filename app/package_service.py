...
def import_package(path: Path, store: Store) -> dict:
    ...
    with zipfile.ZipFile(path, 'r') as z, tempfile.TemporaryDirectory(prefix='vtab_pkg_') as td:
        ...
        if ext in ('.vtabapp', '.vtabpkg'):
            ...
            # Imported projects always become the active local authoring project.
            project['id'] = 'current'
            store.save_project(project)
            store.log('package.import', {'format': ext, 'project': project.get('name'), 'tables': [t['name'] for t in imported_tables]})
        else:
