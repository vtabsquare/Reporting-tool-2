...
class Store:
            ...
            try:
                with self.conn() as c:
                    ...
                    CREATE TABLE IF NOT EXISTS projects(
                        id TEXT PRIMARY KEY,name TEXT,app_theme TEXT DEFAULT 'vtab',app_accent TEXT DEFAULT '#2563eb',ui_density TEXT DEFAULT 'comfortable',model_json TEXT,report_json TEXT,transform_json TEXT,security_json TEXT
                    );
    ...
    def get_project(self,pid='current'):
        with self.conn() as c:
            ...
            d=dict(r)
            for k in ['model_json','report_json','transform_json','security_json']:d[k[:-5]]=json.loads(d[k])
    ...
    def save_project(self,p):
        with self.conn() as c:
            c.execute("""INSERT INTO projects(id,name,app_theme,app_accent,ui_density,model_json,report_json,transform_json,security_json) VALUES(?,?,?,?,?,?,?,?,?)
            ON CONFLICT(id) DO UPDATE SET name=excluded.name,app_theme=excluded.app_theme,app_accent=excluded.app_accent,ui_density=excluded.ui_density,model_json=excluded.model_json,report_json=excluded.report_json,transform_json=excluded.transform_json,security_json=excluded.security_json""",
            (p['id'],p['name'],p.get('appTheme','vtab'),p.get('appAccent','#2563eb'),p.get('uiDensity','comfortable'),json.dumps(p['model']),json.dumps(p['report']),json.dumps(p['transform']),json.dumps(p['security'])))
