...
export default function SecurityWorkbench(){
 ...
 const deleteRole=async(id:string)=>{if(!confirm('Delete this RLS role?'))return;const next=structuredClone(project);next.security.roles=(next.security.roles||[]).filter((r:any)=>r.id!==id);if(next.security.activeRoleId===id)next.security.activeRoleId=null;await persist(next);if(editing?.id===id)setEditing(null);setMessage('Role deleted.')};
 const setActive=async(id:string)=>{const next=structuredClone(project);next.security.activeRoleId=id||null;await persist(next);setMessage(id?'Role simulation enabled. Every authoring query now uses this RLS role.':'Role simulation disabled.')};
